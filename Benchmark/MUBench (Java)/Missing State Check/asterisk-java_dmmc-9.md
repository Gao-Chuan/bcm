...
...
    ...
    logger.error("Unable to get constructor of " + eventClass.getName(), ex);
    throw new RuntimeException("Unable to get constructor of " + eventClass.getName(), ex);
}
```

### Description

If there is an exception when getting the constructor of the event class, the program just logs the error and return null. It might be better to throw a RuntimeException to notify the calling program, not just return null silently.
...
...
    ...
    logger.error("Unable to create new instance of " + eventClass.getName(), ex);
    throw new RuntimeException("Unable to create new instance of " + eventClass.getName(), ex);
}
```

### Description

If there is an exception when creating a new event instance, the program just logs the error and return null. Similar with the previous case, it might be better to throw a RuntimeException to notify the calling program, not just return null silently.
